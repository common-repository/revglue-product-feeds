...
if( !empty ( $rows ) )
{
	$jsonDataCategories = json_decode( wp_remote_retrieve_body( wp_remote_get( RGPFEEDS_API_URL . "api/deal_categories/json/".$project_detail[0]->subcription_id, array( 'timeout' => 120, 'sslverify'   => false ) ) ), true); 
	...
	$jsonData = json_decode( wp_remote_retrieve_body( wp_remote_get( RGPFEEDS_API_URL . "api/deal_stores/json/".$project_detail[0]->subcription_id, array( 'timeout' => 120, 'sslverify'   => false ) ) ), true); 
...
{
	$banner_table = $wpdb->prefix.'rg_banner';
	$jsonDataBanners = json_decode( wp_remote_retrieve_body( wp_remote_get( RGPFEEDS_API_URL . "api/banners/json/".$project_detail[0]->subcription_id, array( 'timeout' => 120, 'sslverify'   => false ) ) ), true); 
